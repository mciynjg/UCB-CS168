...
def traceroute(sendsock: util.Socket, recvsock: util.Socket, ip: str) \
    ...
    for ttl in range(1, TRACEROUTE_MAX_TTL+1):
        current_routers = []
        i = 0
        while i < PROBE_ATTEMPT_COUNT and ip not in current_routers:
            sendsock.set_ttl(ttl)
            sendsock.sendto("Hello".encode(),(ip,TRACEROUTE_PORT_NUMBER))
            if(recvsock.recv_select()):
                data, addr = recvsock.recvfrom()
                ip_header = IPv4(data)
                srcip = ip_header.src
                protoc = ip_header.proto
                ICMP_header = ICMP(data[ip_header.header_len:ip_header.header_len+4])
                SendIP_header = IPv4(data[ip_header.header_len+8:ip_header.header_len+8+20])
                
                
                if(srcip in current_routers):
                    continue
                if(addr[0] == None):
                    continue
                if(protoc != 1):
                    continue
                if(ICMP_header.type != 11 and ICMP_header.type != 3):
                    continue
                if(ICMP_header.code != 0 and ICMP_header.type == 11):
                    continue
                if(len(data) < ip_header.length):
                    continue
                if(ip_header.header_len > 20):  
                    continue
                if(SendIP_header.ttl != ttl):
                    i-=1
                    continue
                current_routers.append(srcip)
                if srcip == ip:
                    util.print_result(current_routers, ttl)
                    print("Destination reached")
                    return routers
            else:
                current_routers.append("*")
        
        routers.append(current_routers)
        util.print_result(current_routers, ttl)
    return routers
