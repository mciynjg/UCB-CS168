...
class StudentUSocket(StudentUSocketBase):
  ...
  def handle_accepted_fin(self, seg):
    ...
    if(self.state == ESTABLISHED):
      self.rcv.nxt = seg.seq|PLUS|len(seg.payload)
      self.set_pending_ack()
    ...
    ## Start of Stage 7.2 ##
    if(self.state == FIN_WAIT_1 and seg.FIN and seg.ACK):
      self.set_pending_ack()
      self.start_timer_timewait()
    elif(self.state == FIN_WAIT_1 and seg.FIN and not seg.ACK):
      self.set_pending_ack()
      self.state = CLOSING
    elif(self.state == FIN_WAIT_2 and seg.FIN):
      self.rcv.nxt=seg.seq |PLUS| len(seg.payload)|PLUS|1
      self.set_pending_ack()
      self.start_timer_timewait()
  ...
  def check_ack(self, seg):
    ...
    if self.state in (ESTABLISHED, FIN_WAIT_1, FIN_WAIT_2, CLOSE_WAIT, CLOSING):
      ## Start of Stage 4.1 ##
      if seg.ack |GT| snd.una and seg.ack|LE|(snd.nxt):
  ...
  def handle_accepted_seg(self, seg, payload):
    ...
    ## Start of Stage 2.2 ##
    if ((self.state in (ESTABLISHED,FIN_WAIT_1,FIN_WAIT_1)) and len(payload)):
  ...
  def maybe_send(self):
    ...
    while remaining > 0:
      if not self.tx_data:
        break
      min =self.mss if remaining|GT|self.mss else remaining
      if min |LT| len(self.tx_data):
        payload = self.tx_data[:min]
      else:
        payload = self.tx_data
      self.tx_data=self.tx_data[min:]
      p = self.new_packet(ack = True,data = payload,syn=False )
      self.tx(p)
      remaining=remaining|MINUS|len(payload)
      num_pkts += 1
